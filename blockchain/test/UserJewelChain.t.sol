...
contract UserJewelChainTest is Test, UserConstant {
  ...
  function test_CreateUser() public {
    ...
    emit IUserJewelChain.UsersJewelsChain__UserCreated(
      USER,
      JEWEL_FACTORY_ROLE_TEST,
      true
    );
  ...
  function test_CreateUserThatExist() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        IUserJewelChain.UsersJewelsChain__UserExisted.selector,
        USER
      )
    );
    userJewelChain.createUser(USER, JEWEL_FACTORY_ROLE_TEST);
  ...
  function test_CreateUserWithNotExistRole() public {
    vm.expectRevert(
      abi.encodeWithSelector(
        IUserJewelChain.UsersJewelsChain__UserInvalidRole.selector,
        USER,
        keccak256('FALSE_ROLE')
      )
    );
  ...
  function test_UpdateUserThatNotExist() public {
    vm.expectRevert(
      abi.encodeWithSelector(
        IUserJewelChain.UsersJewelsChain__UserNotFound.selector,
        USER
      )
    );
    userJewelChain.updateStatusUser(USER, false);
  ...
  function test_UpdateUserSameStatus() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        IUserJewelChain.UsersJewelsChain__UserHasSameStatus.selector,
        USER,
        true
      )
    );
  ...
  function test_DeleteUserThatNotExist() public {
    vm.expectRevert(
      abi.encodeWithSelector(UsersJewelsChain__UserNotFound.selector, USER)
    );
    userJewelChain.deleteUser(USER);
  }
